...
**The active coding-session monitor.** Long-running delegated work is where false confidence loves to hide. Jobs can stall, fail quietly, or appear finished before the output is actually verified. Monitoring those sessions is how the system avoids treating "seems done" as "is done."
...
What makes these heartbeats worth running is that each one prevents a recognizable failure mode: missed updates, lost ideas, repeated mistakes, silent security drift, long-running work nobody is really watching. The stack is opinionated about these because they create real leverage — they reduce risk and increase preparedness without demanding constant human attention.
